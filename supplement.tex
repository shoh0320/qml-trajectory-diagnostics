\documentclass[aps,pra,onecolumn,superscriptaddress,10pt]{revtex4-2}
...
Auditing Trajectory-Based Transfer Diagnostics
in Variational Quantum Learning}
...
identity check (Sec.~\ref{sm:kstep}), the association of alignment with the query loss rather than
...
initialization (Sec.~\ref{sm:robust}); and a synthetic implementation check showing that the joint adjustment does not mechanically zero an explicitly injected direct $A$ component (Sec.~\ref{sm:positive}). All quantities use the definitions, estimators, and
...
$q=\|\gtr\|\|\gval\|$. The small-relative-variation limit of $q$ is a statement about the
...
$\Ival$ approaches, in the small-relative-variation limit, that of $A$.
...
directly. At $K=10$ the correlation is lower and rises with scale ($0.60$ to $0.85$): as gradient magnitudes and adaptation steps shrink while norm variability also changes, the full trajectory moves closer to its initial tangent, and
...
\section{Alignment versus the query loss}
\label{sm:heldout}

The main text controls the apparent association between $A$ and the observable-generalization gap
$G$. The same conclusion holds when the target is the query loss $\Lval(\thetaK)$ itself rather
...
The raw association is moderate and negative but is near zero under the joint control for each pooled $n$
($|\rho|\le0.10$), so the vanishing residual is not special to the signed gap.

\begin{table}[h]
\caption{\label{tab:heldout}%
Alignment versus the query loss $\Lval(\thetaK)$: raw rank correlation, controlling for the local
...
first-order identity predicts. This regime is empirically a near-zero-mean-transfer setting under the specified construction:
...
To rule out that the gap is driven by an unbalanced split, we stratify the support/query split
...
$+0.13$) throughout, so the observed identity-link and adjusted-null association pattern is not specific to the XYZ family or the linear chain.
...
sources of the apparent association.
...
adjusted-null pattern are thus consistent with the first-order variational geometry rather than specific to the
meta-initialization.

\section{Synthetic implementation check: injected components are not mechanically zeroed}
\label{sm:positive}

Every demonstration in the main text is a collapse case, so one may ask whether the joint control is
simply too aggressive---whether it would erase an explicitly injected alignment--gap component.
...
the two controlled channels. In the \emph{direct-channel} construction a direct term $-0.5\,A$ is
...
direct-channel association survives it ($-0.90\to-0.62$; per-seed range $[-0.66,-0.58]$). The numerical values depend on the injected channel strength: sweeping the direct coefficient from
...
controls of the audit protocol therefore remove channel-mediated associations while not mechanically forcing an explicitly injected component to zero: a diagnostic that survives them remains a candidate requiring independent validation.

\begin{table}[h]
\caption{\label{tab:positive}%
Synthetic implementation check. Rank correlation of alignment with the gap, raw and under the joint
control, when the gap contains only the local-improvement and baseline channels versus when an explicitly injected direct alignment channel is added ($3$ seeds $\times150$ tasks, pooled).}
\begin{ruledtabular}
\begin{tabular}{lccc}
construction & $\rho(A,G)$ & $\rho(A,G\,|\,\Ival)$ & $\rho(A,G\,|\,\Ival,\Lval^{0})$ \\
\colrule
channels only    & $-0.65$ & $-0.01$ & $-0.06$ \\
direct channel   & $-0.90$ & $-0.45$ & $-0.62$ \\
\end{tabular}
\end{ruledtabular}
\end{table}
