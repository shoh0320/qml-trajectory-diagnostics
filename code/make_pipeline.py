...
box(ax, xR, yMid, BW, BH + 0.15,
    "Diagnostic audit controls\n" + r"partial corr. $\cdot$ shuffle" + "\n" +
    r"linearization $\cdot$ seeds", c_ctrl)
# ---- verdict (under controls) ----
box(ax, xR, yBot, BW, BH,
    "Interpretation\n" + r"collapse $\Rightarrow$ channel-accounted" + "\n" + r"survive $\Rightarrow$ needs indep. validation", c_verdict)
...
lbl(ax, xR + 0.95, (yMid - (BH+0.15)/2 + yBot + BH/2)/2, "outcome", ha="left")
# 6) physical activators -> verdict (they interpret an association); dashed = interpretive
...
ax.add_patch(FancyArrowPatch((xR, 0.75), (xR, yBot - BH/2), arrowstyle="-|>",
             mutation_scale=12, lw=1.05, color=gold, zorder=1, shrinkA=0, shrinkB=0, linestyle=dash))
lbl(ax, (xL + xR)/2, 0.75, "interpret an association", color="#8a7420")

out_dir = Path("figs"); out_dir.mkdir(parents=True, exist_ok=True)
plt.savefig(out_dir / "fig1_pipeline.pdf", bbox_inches="tight")
plt.savefig(out_dir / "_pipeline_preview.png", bbox_inches="tight", dpi=150)
plt.close()
