...
def meta_eval(dev, n, L, ops, theta0, rng, n_eval, K, inner_lr, coeff_kw):
    ...
    for t in range(n_eval):
        ...
        G = LvalK - LtrK            # query--support observable gap (primary target)
...
def print_report(summary, barren):
    ...
    print("  * rho(A,G) small and flipping sign across seeds       => alignment does not track the query-support gap in this run.")
    print("  If these hold, the scale trends reported in the paper are reproduced.")
